...
var testrm Manager = &resembleManager{
	core:            testrc,
	generatedSpeech: platform.NewExpiringCache(platform.NewId("test-resemble-speech-"), 2),
	signals:         make(map[string]chan struct{}),
}

func TestGetResembleManager(t *testing.T) {
	_ = GetResembleManager()
}

func TestResembleGenerateRetrieveSpeech(t *testing.T) {
	...
	start1 := time.Now()
	testrm.GenerateSpeech(ctx, profileId, speechId1, text1)
	_, err := testrm.GeneratedSpeech(ctx, speechId1)
	end1 := time.Now()
